...
class Observables:
    ...
    def __init__(self, params):
        self.__dict__.update(params.control.__dict__)
        self.__dict__.update(params.__dict__)
        useless = ['potential', 'PostProcessing', 'Magnetic', 'control', 'integrator', 'thermostat',
                   'BC', 'Langevin']
        for a in useless:
            delattr(self, a)

        if hasattr(params, 'pppm'):
            del self.pppm

        # Create the lists of k vectors
        if hasattr(params.PostProcessing, 'no_ka_vectors'):
            if len(params.PostProcessing.no_ka_vectors) == 0:
                self.no_ka_vectors = np.array([params.PostProcessing.no_ka_values,
                                               params.PostProcessing.no_ka_values,
                                               params.PostProcessing.no_ka_values], dtype=int)
            else:
                self.no_ka_vectors = params.PostProcessing.no_ka_vectors  # number of ka values
        else:
        ...
        self.species_np = np.zeros(self.num_species, dtype=int)
        self.species_names = []
        self.species_wp = np.zeros(self.num_species)
        self.species_charge = np.zeros(self.num_species)
        ...
        for i, sp in enumerate(params.species):
            ...
            self.species_num_dens[i] = sp.num_density

        self.tot_mass_density = self.species_masses.transpose() @ self.species_num_dens

        if hasattr(params.control, 'mpl_style'):
            plt.style.use(params.control.mpl_style)
...
class CurrentCorrelationFunctions(Observables):
    ...
    def compute(self):
        ...
        # Calculate Lkw
        Lkw = calc_Skw(vkt, self.k_list, self.k_counts, self.species_np, self.prod_no_dumps, self.dt,
                       self.prod_dump_step)
        Tkw_i = calc_Skw(vkt_i, self.k_list, self.k_counts, self.species_np, self.prod_no_dumps, self.dt,
                         self.prod_dump_step)
        Tkw_j = calc_Skw(vkt_j, self.k_list, self.k_counts, self.species_np, self.prod_no_dumps, self.dt,
                         self.prod_dump_step)
        Tkw_k = calc_Skw(vkt_k, self.k_list, self.k_counts, self.species_np, self.prod_no_dumps, self.dt,
                         self.prod_dump_step)
   ...
   """
    ...
    def compute(self):
        ...
        try:
            ...
            nkt = calc_nkt(self.prod_dump_dir, self.prod_no_dumps, self.prod_dump_step, self.species_np, self.k_list)
            np.save(self.nkt_file, nkt)

        # Calculate Skw
        Skw = calc_Skw(nkt, self.k_list, self.k_counts, self.species_np, self.prod_no_dumps, self.dt,
                       self.prod_dump_step)
        print("Saving S(k,w)")
        sp_indx = 0
        for i, sp1 in enumerate(self.species):
...
class RadialDistributionFunction(Observables):
    ...
    def __init__(self, params):
        ...
        self.dr_rdf = params.potential.rc / self.no_bins
...
class StaticStructureFactor(Observables):
    ...
    def compute(self):
        ...
        Sk_all = calc_Sk(nkt, self.k_list, self.k_counts, self.species_np, self.prod_no_dumps)
...
class VelocityAutocorrelationFunctions(Observables):
    ...
    def compute(self, time_averaging=False, it_skip=100):
        ...
        if self.num_species > 1:
            ...
            vacf = calc_vacf_single(vel, self.species_np, time_averaging, it_skip)
        # Save to csv
        v_ij = 0
        for i, sp1 in enumerate(self.species):
...
class VelocityMoments(Observables):
    ...
    def compute(self):
        ...
        ratios = calc_moment_ratios(moments, self.species_np, self.prod_no_dumps)
        # Save the dataframe
        for i, sp in enumerate(self.species):
...
class XYZWriter:
    ...
    def __init__(self, params):
        ...
        params: S_params class
            Simulation's parameters.
        """
        self.saving_dir = params.control.job_dir
        self.prod_dump_dir = params.control.prod_dump_dir
        self.filename = os.path.join(self.saving_dir, "pva_" + params.control.job_id + '.xyz')
        self.dump_skip = 1
...
def read_pickle(job_dir):
    ...
    pickle_file = os.path.join(job_dir, "S_parameters.pickle")
