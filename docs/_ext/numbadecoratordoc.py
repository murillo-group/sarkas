...
into the docstring.
"""
# Thanks to @HPLegion. From https://github.com/numba/numba/issues/5755
# Note: At the moment the prefix is not working. Is it maybe due to the
# fact that we are using `autosummary` rather than `autodoc`? 
...
# Note: We are currently not using the Vectorized decorator, so the
# `VectorizedDocumenter` and `VectorizeDirective` classes are not needed.
...
class JitDirective(PyFunction):
    ...
    def get_signature_prefix(self, sig):
        print(f"\n\n\tJITDirective \n{type(self.env.config.numba_jit_prefix)=}\n{self.env.config=}\n\n")
