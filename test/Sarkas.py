...
if not (params.load[0].method == "restart"):
    ...
    for it in range(Neq):
        ...
        print(it, pos[0, 0], vel[0, 0], acc[0, 0])
#---------------
        K = 0.5*mi*np.ndarray.sum(vel**2)
        Tp = (2/3)*K/float(N)/const.kb
        if(glb.units == "Yukawa"):
...
if(0):
        f10 = "Restart1/restart_10.npz"
        f11 = "Restart1/restart_11.npz"
        d10 = np.load(f10) 
        d11 = np.load(f11) 
        pos10 = d10['pos']
        vel10 = d10["vel"]
...
if(0):
    print(np.array_equal(pos, pos10))
    print(np.array_equal(vel, vel10))
    print(np.array_equal(acc, acc10))
    pos, vel, acc, U = velocity_verlet.update_Langevin(pos, vel, acc, Z, G_k, kx_v, ky_v, kz_v, acc_s_r, acc_fft, rho_r, E_x_p, E_y_p, E_z_p)
    print(np.array_equal(pos, pos11))
    print(np.array_equal(vel, vel11))
    print(np.array_equal(acc, acc11))
    sys.exit()
...
#    print(it, np.array_equal(vel, vel10), np.array_equal(vel, vel11))
